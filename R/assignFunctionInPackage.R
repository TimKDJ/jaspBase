...
fakeGettext <- function(..., domain = NULL) {
  if (missing(domain)) {
    pkgName <- packageName(sys.frame(-1))
    if (!is.null(pkgName))
      domain <- paste0("R-", pkgName)
  }
  args <- lapply(list(...), as.character)
  .Internal(gettext(domain, unlist(args)))
}

fakeGettextf <- function(fmt, ..., domain = NULL) {
  if (missing(domain)) {
    pkgName <- packageName(sys.frame(-1))
    if (!is.null(pkgName))
      domain <- paste0("R-", pkgName)
...
fakeNgettext <- function(n, msg1, msg2, domain = NULL) {
  if (missing(domain)) {
    pkgName <- packageName(sys.frame(-1))
    if (!is.null(pkgName))
      domain <- paste0("R-", pkgName)
